...
export const ProviderButton = ({ name, children, className, ...props }: ProviderButtonProps) => (
  <button
    ...
    type="submit"
    className={cn(
      `enabled:animate-fade-in-slow border-providers mt-2 flex w-full flex-row justify-center gap-3 rounded-2xl border border-solid bg-white p-2 align-middle ${robotoFont.className} transition duration-500 enabled:cursor-pointer enabled:hover:-translate-y-1 enabled:hover:shadow-lg disabled:bg-gray-100`,
      className
    )}
  >
